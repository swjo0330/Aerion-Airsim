...
# 2026-07-07 Town10HD_Opt 클릭 미션 성공 상태 고정

현재 성공한 실행 경로는 `scripts/run_clicked_mission_experiment.sh` 이다.
...
- 원본 미션: `recordings/missions/clicked_mission.json`
- 실행용 변환 미션: `recordings/missions/clicked_mission_current_home_fcu_axes.json`
- AirSim NED 원점: `recordings/maps/airsim_ned_origin.json`
- 기존 정상 소환 AirSim offset: `X=240`, `Y=-170`, `Z=0.2`
- 실제 map/FCU home: `east=13.859121`, `north=67.803418`
- 노드1 상대거리: 약 `38.57m`

핵심 주의사항:

- `X=240`, `Y=-170`은 CARLA/OpenDRIVE 지도 좌표가 아니다.
- 이 값은 AirSim `Vehicles.drone1.X/Y`에 들어가는 global-NED offset이다.
- 실제 미션 home은 `AirSim NED 원점 + spawn offset`으로 계산해야 한다.
- 계산은 `scripts/compute_map_home_for_airsim_spawn.py`에서 수행한다.
- `run_clicked_mission_experiment.sh`는 기본 `SPAWN_MODE=previous_home`으로 이 계산을 자동 수행한다.
...
따라서 성공한 home 계산은 다음과 같다.

```text
east  =  183.85912109 - 170  = 13.85912109
north = -172.19658203 + 240  = 67.80341797
```

문제가 다시 발생하면 먼저 확인할 것:

1. UE 로그에 `AERION_NED_ORIGIN_UE`와 `AERION_SPAWN`이 찍히는지 확인한다.
2. `recordings/maps/airsim_ned_origin.json`이 현재 UE Play 세션과 맞는지 확인한다.
3. UE PlayerStart나 AirSim plugin을 바꿨다면 UE를 완전히 종료하고 다시 실행한다.
4. AirSim plugin 소스를 바꿨다면 `CarlaUnrealEditor`를 재빌드해야 한다.
