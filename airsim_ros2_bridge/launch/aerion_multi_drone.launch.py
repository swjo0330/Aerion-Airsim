...
기본값은 master(drone1)만 camera=true, 모든 드론 range=true. 카메라를 전체 활성화하려면
camera_mode:=all 을 명시한다.
...
def _build_nodes(context, *args, **kwargs):
    ...
    for n in range(1, drone_count + 1):
        ...
        per_vehicle_camera = enable_camera and (camera_mode == 'all' or n == 1)
        nodes.append(Node(
            ...
            parameters=[{
                ...
                'enable_camera': per_vehicle_camera,
...
def generate_launch_description():
    args = [
        DeclareLaunchArgument('drone_count', default_value='3',
                              description='1~5대 (settings.json의 drone1..droneN와 일치해야 함)'),
        DeclareLaunchArgument('airsim_ip', default_value='127.0.0.1'),
        DeclareLaunchArgument('airsim_port', default_value='41451'),
        DeclareLaunchArgument('enable_camera', default_value='true'),
        DeclareLaunchArgument('camera_mode', default_value='master',
                              description='master: drone1 only | all: every drone'),
